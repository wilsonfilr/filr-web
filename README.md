...
- **Apple:** enable the Apple provider in Supabase (Services ID + `.p8` secret key). For **web**, create a
  **Services ID** in [Apple Developer](https://developer.apple.com/account/resources/identifiers/list/serviceId)
  (separate from the iOS App ID `com.wilsonfilr.filr`). Configure **Sign in with Apple** on that Services ID:

  1. **Domains and Subdomains:** `web.myfilr.app` and `jjuokcotwuzxiejicchd.supabase.co` (your Supabase project host)
  2. **Return URLs** (add all of these):
